# motif finding utils, modified from DNABERT-viz utils
import os
import pandas as pd
import numpy as np
...
def merge_motifs(motif_seqs, min_len=5, align_all_ties=True, **kwargs):
    ...
    **kwargs -- other input arguments, may include:
    ...
    merged_motif_seqs = {}
    for motif in tqdm(sorted(motif_seqs, key=len)): # query motif
...
def motif_analysis(pos_seqs,
    ...
    **kwargs -- other input arguments, may include:
    ...
    for i, score in enumerate(tqdm(pos_atten_scores)):
        seq_len = len(pos_seqs[i])
        # print(seq_len,len(score))
        ...
        # print('motif_regions',motif_regions)
    ...
    if save_file_dir is not None:
        ...
        with open(save_file_dir+'/all_motif.fa', 'w') as f_all:
            for motif, instances in tqdm(merged_motif_seqs.items()):
                ll_ins = len(instances['seq_idx'])
                f_all.write(f'>{motif}_{ll_ins}\n')
                # saving to files
